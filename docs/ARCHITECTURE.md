...
Tài liệu này trình bày thiết kế kiến trúc monolithic, cấu trúc thư mục thực tế áp dụng mẫu thiết kế **Repository Pattern & Unit of Work**, luồng dữ liệu thông qua cơ chế **Dependency Injection (DI)** và giải pháp kết nối trực tiếp với **SQL Server** trong hệ thống **DocConnect**.
...
* **Specific Repositories (như `IDoctorRepository`, `IAppointmentRepository`):** Kế thừa từ Generic Repository và bổ sung các phương thức truy vấn đặc thù (ví dụ: tìm bác sĩ theo chuyên khoa, kiểm tra lịch hẹn trùng ca khám).
...
* **Lưu ý về Quản lý User:** Hệ thống sử dụng **ASP.NET Core Identity** để quản lý tài khoản. Do đó, các nghiệp vụ liên quan đến User/Account sẽ tương tác trực tiếp với lớp `UserManager<ApplicationUser>` và `SignInManager<ApplicationUser>` mặc định của ASP.NET Core thay vì viết thêm một UserRepository dư thừa.

---

## 2. Cấu Trúc Thư Mục Thực Tế (Project Folder Structure)

Dưới đây là cấu trúc thư mục thực tế của dự án. Một số thư mục/tập tin nghiệp vụ (như Repositories chi tiết, Services) đã được định hình sẵn cấu trúc hoặc sẽ được nhóm tạo thêm trong quá trình triển khai:

```text
DocConnect/
├── DocConnect.slnx                     # Solution quản lý dự án (Cấu trúc mới của VS)
...
└── DocConnect.Web/                     # Dự án Monolithic chính (ASP.NET Core Web App)
    ...
    │   ├── ApplicationDbContext.cs     # Lớp DbContext kế thừa từ IdentityDbContext<ApplicationUser>
    │   └── Migrations/                 # Quản lý nâng cấp lược đồ SQL Server
    │
    ├── Repositories/                   # Tầng Repository (Thư mục gốc của dự án Web)
    │   ├── Interfaces/                 # Định nghĩa các Interface Repository
    ...
    │
    ├── UnitOfWork/                     # Quản lý Transaction & SaveChanges (Thư mục gốc của dự án Web)
    │   ├── IUnitOfWork.cs
    │   └── UnitOfWork.cs
    │
    ...
    │   ├── HomeController.cs
    │   ├── AccountController.cs        # Quản lý Đăng ký (2 tab)/Đăng nhập (sử dụng UserManager)
    │   ├── AppointmentController.cs    # Quản lý Đặt lịch khám và duyệt lịch khám
    │   └── ConsultationController.cs   # Điều hướng phòng khám và xử lý API Chat AJAX
    │
    ├── Models/                         # Model dùng cho ứng dụng
    │   ├── Entities/                   # Định nghĩa các thực thể CSDL (Patient, Doctor, Specialty...)
    │   ├── ViewModels/                 # ViewModels chứa dữ liệu phục vụ hiển thị lên Razor Views
    │   └── ErrorViewModel.cs
    │
    ├── Views/                          # Razor MVC Views (Giao diện Bootstrap)
    └── wwwroot/                        # Tệp tĩnh (CSS, JS, Images, Bootstrap)
```
...
Các thành phần Repository, Unit of Work và Service sẽ được đăng ký trong `Program.cs` thông qua phương thức `AddScoped` (vòng đời tồn tại theo từng HTTP Request).

Ví dụ đoạn mã cấu hình DI tiêu biểu sẽ được triển khai:
```csharp
...
// 3. Đăng ký Generic Repository và Unit of Work
builder.Services.AddScoped<IUnitOfWork, UnitOfWork>();
builder.Services.AddScoped(typeof(IGenericRepository<>), typeof(GenericRepository<>));

// 4. Đăng ký các Specific Repositories
builder.Services.AddScoped<IDoctorRepository, DoctorRepository>();
builder.Services.AddScoped<IPatientRepository, PatientRepository>();
builder.Services.AddScoped<IAppointmentRepository, AppointmentRepository>();

// 5. Đăng ký tầng Business Logic Services
...
* **API Nhắn tin:** Client gửi tin nhắn mới (văn bản hoặc hình ảnh dạng file) thông qua yêu cầu AJAX POST đến `ConsultationController`. Controller nhận tin nhắn, lưu trữ hình ảnh vào `wwwroot/uploads` (nếu có), lưu thông tin tin nhắn vào CSDL thông qua `IUnitOfWork`.
...
* **SQL Injection Prevention:** Tránh viết các câu lệnh SQL thuần ghép chuỗi. Tất cả các truy vấn dữ liệu phải thực hiện thông qua LINQ hoặc parameterized raw SQL.
